...
function OrderNewLand() {
  ...
  useEffect(() => {
    ...
    getAllSubscriptionsPlans().then((res) => {
      dispatch({
        type: "SET_ALL_PREVELEGIOS_FOR_ACTIVE_TARIFF",
        payload: res.data,
      });
    });
  }, []);
  return (
    <div className="newLand">
      <GoBackButton classname="buttonBack" />
      <div className="newLand__content">
        <div className="newLand__content__description">
          Select subscription and your legacy room
        </div>
        <div className="newLand__content__info">
          <StringSlider
            array={allPrivelegiusForActiveTariff}
            onChange={onTypeChange}
          />
          <Info info={allPrivelegiusForActiveTariff[currTypeLagacyRoomId]} />
          <MoreInfo
            moreInfo={allPrivelegiusForActiveTariff[currTypeLagacyRoomId]}
          />
          <BlueCustomBtn
            text="CONTINUE"
            onClick={onContinue}
